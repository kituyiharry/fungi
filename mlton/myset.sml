...
functor TreeSet(structure Ord: OrderedType) :> TSet where type t = Ord.t  = struct
    ...
    fun mem (aval: Ord.t) (s: t set) = case s of
      ...
      *)
   fun take_min (s: t set) = case s of
         Empty => (NONE, Empty)
         | Node(Empty, v, r) => (SOME v, r)
         | Node(l, v, r) => let val (el, rest) = take_min l in
             (el, Node(rest, v, r))
         end
    ...
    *)
     ...
     fun take_root (s: t set): (t option * t set) = case s of
         Empty =>  (NONE, Empty)
         | Node (x, v, y) => 
             let val g = let val (min, rest) = take_min y in 
                 case min of
    ...
    fun of_list (s: t list): t set = case s of
     ...
     (*fun of_seq (s: t seq) = STREAM.fold_left (fn x a => add a x) Empty*)
    ...
    fun remove (el: t) (s: t set) = case s of
        ...
        | Node (x, a, y) => 
            ...
            EQUAL => let val (min, rest) = take_min y in
                case min of
    ...
    fun for_all (f: t -> bool) (s: t set) = case s of
        Empty => true
        | nodes => 
            let val (max, rest) = take_min nodes in 
              case max of
